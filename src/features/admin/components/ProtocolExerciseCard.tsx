...
export const ProtocolExerciseCard: React.FC<ProtocolExerciseCardProps> = React.memo(({
  ...
  const replaceTaxonomy = useMemo(
    () => buildExerciseFilterGroups(exerciseLibrary || []),
    [exerciseLibrary],
  );

  const replaceMainGroups = useMemo(
    () => ['Sugestões', 'Todos', ...replaceTaxonomy.map((group) => group.name)],
    [replaceTaxonomy],
  );

  const replaceActiveMainGroup = useMemo(() => {
    if (replaceActiveTag === 'Sugestões' || replaceActiveTag === 'Todos') return replaceActiveTag;
    return replaceTaxonomy.find((group) => (
      group.name === replaceActiveTag
      || group.subgroups.some((subgroup) => subgroup.name === replaceActiveTag)
    ))?.name || 'Todos';
  }, [replaceActiveTag, replaceTaxonomy]);

  const replaceSubGroups = useMemo(() => {
    if (replaceActiveMainGroup === 'Sugestões' || replaceActiveMainGroup === 'Todos') {
      return [];
    }
    return replaceTaxonomy.find((group) => group.name === replaceActiveMainGroup)
      ?.subgroups.map((subgroup) => subgroup.name) || [];
  }, [replaceActiveMainGroup, replaceTaxonomy]);
  ...
  const filteredReplaceLibrary = useMemo(() => {
    ...
    let candidates = exerciseLibrary.filter(ex => ex.id !== exercise.exercise_id && ex.is_active !== false);
    ...
    if (replaceActiveTag === 'Sugestões') {
      ...
      candidates = candidates.filter(ex => exerciseMatchesMuscleFilter(ex, replaceActiveTag));
    }

    // Apply text query filtering if present
    if (replaceSearchQuery.trim()) {
      const q = normalizeExerciseFilterText(replaceSearchQuery);
      candidates = candidates.filter(ex => buildExerciseSearchText(ex).includes(q));
    }

    // Sort candidates
    if (replaceActiveTag === 'Sugestões') {
      return candidates
        .map((item) => ({ item, score: getReplacementScore(item) }))
        .sort((a, b) => b.score - a.score || a.item.name.localeCompare(b.item.name, 'pt-BR'))
        .slice(0, 8);
    } else {
      if (replaceActiveTag !== 'Todos') {
        candidates = candidates.sort((a, b) => a.name.localeCompare(b.name, 'pt-BR'));
      } else {
        candidates = candidates.sort((a, b) => a.name.localeCompare(b.name));
      }
      return candidates.slice(0, 40).map((item) => ({ item, score: undefined }));
    }
