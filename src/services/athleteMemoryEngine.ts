...
let athleteMemoryWarningShown = false;
let performanceMemoryWarningShown = false;

const isMissingTableError = (error: any) => {
  const message = `${error?.message || ''} ${error?.details || ''}`.toLowerCase();
  return error?.status === 404 || error?.code === '42P01' || error?.code === 'PGRST205' || message.includes('does not exist') || message.includes('not found');
};

const warnOnce = (kind: 'athlete' | 'performance', error: any) => {
  if (kind === 'athlete') {
    if (!athleteMemoryWarningShown) {
      athleteMemoryWarningShown = true;
      console.warn('[MemoryEngine] athlete_memory remote table unavailable. Using local fallback for this session.', error?.message || error);
    }
    return;
  }

  if (!performanceMemoryWarningShown) {
    performanceMemoryWarningShown = true;
    console.warn('[MemoryEngine] exercise_performance_memory remote table unavailable. Using local fallback for this session.', error?.message || error);
  }
};

// Web Audio API helper to synthesize lightweight UI feedback sounds.
export const playSensoryTone = (type: 'confirm' | 'success' | 'click' | 'focus' | 'pulse' | 'warning') => {
  try {
    ...
    const toneMap = {
      click: { start: 600, end: 1200, gain: 0.04, duration: 0.06, wave: 'sine' as OscillatorType },
      confirm: { start: 880, end: 440, gain: 0.08, duration: 0.16, wave: 'sine' as OscillatorType },
      success: { start: 523.25, end: 783.99, gain: 0.06, duration: 0.45, wave: 'triangle' as OscillatorType },
      focus: { start: 150, end: 100, gain: 0.12, duration: 0.4, wave: 'sine' as OscillatorType },
      pulse: { start: 1000, end: 1000, gain: 0.02, duration: 0.02, wave: 'sine' as OscillatorType },
      warning: { start: 220, end: 180, gain: 0.03, duration: 0.25, wave: 'sawtooth' as OscillatorType },
    }[type];

    osc.type = toneMap.wave;
    osc.frequency.setValueAtTime(toneMap.start, ctx.currentTime);
    osc.frequency.exponentialRampToValueAtTime(Math.max(1, toneMap.end), ctx.currentTime + toneMap.duration);
    gain.gain.setValueAtTime(toneMap.gain, ctx.currentTime);
    gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + toneMap.duration);
    osc.connect(gain);
    gain.connect(ctx.destination);
    osc.start();
    osc.stop(ctx.currentTime + toneMap.duration);
  } catch (e) {
    if (isDev) console.warn('[Sensory Tone] AudioContext play failed', e);
  }
};
...
export const athleteMemoryEngine = {
  async getMemory(userId: string): Promise<AthleteMemory> {
    ...
    const localKey = `coach_rubi_mem_${userId}`;
    ...
    if (athleteMemoryRemoteAvailable !== false) {
      try {
        ...
        if (isMissingTableError(dbError)) {
          athleteMemoryRemoteAvailable = false;
          warnOnce('athlete', dbError);
        } else {
          console.warn('[MemoryEngine] athlete_memory query failed, using local fallback.', dbError);
        }
      }
    }

    const defaultMem = createDefaultMemory(userId);
    localStorage.setItem(localKey, JSON.stringify(defaultMem));
    return defaultMem;
  },
    ...
    try {
      ...
      if (error) {
        if (isMissingTableError(error)) {
          ...
          console.warn('[MemoryEngine] Supabase memory upsert failed, continuing offline.', error.message);
        }
      } else {
        athleteMemoryRemoteAvailable = true;
      }
  ...
  async learnFromWorkoutSession(
    ...
    mem.training_personality = mem.average_rpe >= 8.8
      ? 'Focado em Intensidade (Hardcore)'
      : mem.consistency_score >= 90
        ? 'Consistente Metódico'
        : mem.adaptation_level > 85
          ? 'Atleta Resiliente Adaptativo'
          : 'Buscador de Performance';
  ...
  async trackExerciseSkip(userId: string, exerciseId: string): Promise<void> {
    ...
    if (mem.exercise_skip_patterns[exerciseId] >= 3 && isDev) {
  ...
  async getPerformanceMemory(userId: string, exerciseId: string): Promise<ExercisePerformanceMemory | null> {
    ...
    try {
      ...
      if (isMissingTableError(e)) {
        performanceMemoryRemoteAvailable = false;
        warnOnce('performance', e);
      } else {
        console.warn('[MemoryEngine] exercise_performance_memory query failed, using local fallback.', e);
      }
    }
      ...
      try {
        ...
        if (error) {
          if (isMissingTableError(error)) {
            ...
            console.warn('[MemoryEngine] exercise performance upsert failed, continuing locally.', error.message);
          }
        } else {
          performanceMemoryRemoteAvailable = true;
        }
  ...
  async predictLoadSuggestion(
    ...
    let message = 'Manter carga atual para consolidar técnica.';
