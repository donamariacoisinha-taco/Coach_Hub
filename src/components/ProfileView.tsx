...
const ProfileView: React.FC<ProfileViewProps> = ({ profile, onUpdate }) => {
  ...
  const measurementsQuery = useSmartQuery('body_measurements', async () => {
    return await profileApi.getBodyMeasurements();
  }, {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await profileApi.deleteBodyMeasurement(id);
      refresh();
  ...
  const handleSaveMeasurement = async () => {
    ...
    try {
      ...
      if (editingMeasureId) {
        payload.id = editingMeasureId;
      }

      await profileApi.upsertBodyMeasurement(payload);
      
      const latest = measurements[0];
      if (!latest || new Date(newMeasure.measured_at!) >= new Date(latest.measured_at)) {
        await profileApi.updateWeight(user.id, newMeasure.weight!);
      }
